...
SatelliteFactory.prototype = {
  ...
  findClosest: function(coordinates){
    var nearestSatellite = {
      distance: 100000
    };

    for(var sat in coordinates) {
        distance = this.getDistanceInKilometers(coordinates[sat].lat, coordinates[sat].lon, this.user.coords.latitude, this.user.coords.longitude);
    ...
    var satellite = new SpaceObject(nearestSatellite, this.user);
    satellite.setVisibility(this.user.coords);
  },
    ...
    spaceObject: {
      visibility: {
        "true": {
          ...
          "text": function(user) {
            if (user.darkOut) {
                return "You might be able to see the " + user.spaceObject.name + " to the " + user.spaceObject.direction + "! Tweet to @NASA_Astronauts!";
            } else {
                return user.spaceObject.name + " in the " + user.spaceObject.direction + "! Tweet to @NASA_Astronauts!";
            }
          }
        },
        "false": {
          "title": function(user) {
            return user.spaceObject.name + " is gone!";
          },
          "text": function(user) {
              return "Sorry, the " + user.spaceObject.name + " is no longer visible!";
          }
        }
      }
    }
...
View.prototype = {
  notify: function(user) {
    var isVisible = user.spaceObject.visible.toString();
    var messages = this.messages.spaceObject;
    var title = messages.visibility[isVisible].title(user);
    var text = messages.visibility[isVisible].text(user);
...
User.prototype = {
  ...
  setSpaceObjectProperties: function(spaceObject) {
    if (this.spaceObject.visible != spaceObject.visible || this.spaceObject.direction != spaceObject.direction) {
      ...
      this.darkOut = this.isDarkOut();
      this.spaceObject.name = spaceObject.name;
